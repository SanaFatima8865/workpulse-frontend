...
export interface TRFI       { _id:string;rfiNumber:string;subject:string;question:string;status:string;priority:string;impact:string;costImpact?:number;scheduleDays?:number;submittedById:string;assignedToId?:string;dueDate?:string;answeredDate?:string;responses:Array<{_id:string;responderId:string;content:string;isOfficial:boolean;createdAt:string}>;tags:string[];section?:string;drawingReference?:string;projectId:string;createdAt:string;updatedAt:string }
...
export interface TChangeOrder { _id:string;coNumber:string;title:string;type:string;status:string;costImpact:number;scheduleDays:number;laborCost:number;materialCost:number;description?:string;initiatedById:string;approvedDate?:string;approvedById?:string;rejectionReason?:string;projectId:string;createdAt:string;updatedAt:string }
...
export interface TPunchListItem { _id:string;itemNumber:string;description:string;location:string;status:string;priority:string;assignedTo?:string;responsible:string;dueDate?:string;completedDate?:string;verifiedDate?:string;notes:string;projectId:string;createdAt:string }
...
export const usePunchList         =(pId:string) => {const wsId=useWsId();return useQuery({queryKey:['punch',wsId,pId],queryFn:async()=>(await constructionApi.getPunchList(pId,wsId)).data??[],enabled:!!wsId&&!!pId});};
export const usePunchItems        =usePunchList;
...
export const useUpdatePunchItem   =() => {const wsId=useWsId();const qc=useQueryClient();return useMutation({mutationFn:({id,...d}:{id:string}&Record<string,unknown>)=>constructionApi.updatePunchItem(id,wsId,d),onSuccess:()=>{qc.invalidateQueries({queryKey:['punch',wsId]});},onError:(e)=>toast.error(errMsg(e))});};
